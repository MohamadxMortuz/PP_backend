  ...
  try {
    ...
    try {
      ...
      if (options.filterOnServer) {
        ...
        const memoryCached = cacheService.get(baseCacheKey);
        const dbCached = !memoryCached ? await dbService.getSearchResults(query, baseOptions) : null;
        
        // Normalize: memory cache stores {products, pagination}, DB stores mongoose doc
        const cachedBaseResults = memoryCached || (dbCached ? { products: dbCached.products, pagination: dbCached.pagination } : null);
        
        if (cachedBaseResults && cachedBaseResults.products && cachedBaseResults.products.length > 0) {
